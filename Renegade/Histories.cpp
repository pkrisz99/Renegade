...
void Histories::UpdateCorrection(const Position& position, const int16_t refEval, const int16_t score, const int depth) {
	...
	if (oldPawnUpperBits != newPawnUpperBits) pawnValue /= 2;
	...
	if (oldWhiteNonPawnUpperBits != newWhiteNonPawnUpperBits) whiteNonPawnValue /= 2;
	...
	if (oldBlackNonPawnUpperBits != newBlackNonPawnUpperBits) blackNonPawnValue /= 2;
