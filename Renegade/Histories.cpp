...
void Histories::UpdateQuietHistory(const Position& position, const Move& m, const int level, const int depth) {
	...
	const int delta = std::min(316 * depth, 2433) * (bonus ? 1 : -1);
...
void Histories::UpdateCaptureHistory(const Position& position, const Move& m, const int depth) {
	const int delta = std::min(278 * depth, 2433) * (bonus ? 1 : -1);
...
void Histories::UpdateCorrection(const Position& position, const int16_t refEval, const int16_t score, const int depth) {
	...
	materialValue = ((205 - weight) * materialValue + weight * diff) / 205;
	materialValue = std::clamp(materialValue, -8330, 8330);

	const uint64_t pawnKey = position.GetPawnKey() % 16384;
	int32_t& pawnValue = PawnsCorrectionHistory[position.Turn()][pawnKey];
	pawnValue = ((205 - weight) * pawnValue + weight * diff) / 205;
	pawnValue = std::clamp(pawnValue, -8330, 8330);
	...
	if (position.Moves.size() >= 2) {
		...
		followUpValue = ((205 - weight) * followUpValue + weight * diff) / 205;
		followUpValue = std::clamp(followUpValue, -8330, 8330);
	}
}
