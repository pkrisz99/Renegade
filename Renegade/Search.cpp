...
int Search::SearchRecursive(ThreadData& t, int depth, const int level, int alpha, int beta, const bool cutNode) {
	...
	if (!pvNode && !inCheck && !singularSearch) {
		...
		if (depth <= 10 && !IsMateScore(beta)) {
			const int rfpMargin = depth * 131 - improving * 71;
		...
		if (depth >= 3 && eval >= beta && !position.IsPreviousMoveNull() && position.ZugzwangUnlikely()) {
			...
			const int nmpReduction = [&] {
				const int defaultReduction = 4 + depth / 3 + std::min((eval - beta) / 226, 3);
	...
	while (true) {
		...
		if (singularCandidate && m == ttMove) {
			...
			if (onlyMove) {
				...
				const bool tripleExtend = !pvNode && position.IsMoveQuiet(m) && (singularScore < singularBeta - marginFactor * (187 + std::abs(ttEval) / 8));
		...
		if (depth >= 3 && (legalMoveCount >= (3 + pvNode * 2 + rootNode * 2)) && isQuiet) {
			...
			if (score > alpha && reducedDepth < depth - 1) {
				deepen = score > (bestScore + 34 + depth * 5);
	...
	if (bestScore >= beta && !aborting) {
		...
		if (quietBestMove) {
			t.History.UpdateQuietHistory<Bonus>(position, bestMove, level, depth + pvNode, failHighCount);
			...
			t.History.UpdateCaptureHistory<Bonus>(position, bestMove, depth + pvNode, failHighCount);
		}
		...
		for (const Move& qt : quietsTried) t.History.UpdateQuietHistory<Penalty>(position, qt, level, depth + pvNode, 1);
		for (const Move& ct : capturesTried) t.History.UpdateCaptureHistory<Penalty>(position, ct, depth + pvNode, 1);
	}
