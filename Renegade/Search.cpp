...
int EvaluateThreatScore(const Board& board, const uint64_t whiteAttacks, const uint64_t blackAttacks) {
	const bool turn = board.Turn;

	const int whiteThreatPower = [&]{
		const int raw =
			Popcount(whiteAttacks & board.BlackPawnBits) * 1
			+ Popcount(whiteAttacks & board.BlackKnightBits) * 4
			+ Popcount(whiteAttacks & board.BlackBishopBits) * 4
			+ Popcount(whiteAttacks & board.BlackRookBits) * 6
			+ Popcount(whiteAttacks & board.BlackQueenBits) * 11
			+ Popcount(whiteAttacks & board.BlackKingBits) * 11;
		return std::sqrt(10 * raw);
	}();
