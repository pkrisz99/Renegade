...
void Search::SearchMoves(ThreadData& t) {
	...
	while (!finished) {
		...
		if (t.RootDepth < 5) {
			...
			// Aspiration windows
			int windowSize = Tune::tune_asp_start();
...
int Search::SearchRecursive(ThreadData& t, int depth, const int level, int alpha, int beta, const bool pvNode, const bool cutNode) {
	...
	if (!pvNode && !inCheck && !singularSearch) {
		...
		if (depth <= 7 && !IsMateScore(beta)) {
			const int rfpMargin = std::max(depth * Tune::tune_rfp_margin() - improving * Tune::tune_rfp_improving_reduction(), 0);
		...
		if (depth >= 3 && eval >= beta && !position.IsPreviousMoveNull() && position.ZugzwangUnlikely()) {
			...
			const int nmpReduction = [&] {
				const int defaultReduction = 4 + depth / 3 + std::min((eval - beta) / Tune::tune_nmp_eval_divider(), 3);
		...
		if (depth <= 5 && !IsMateScore(beta)) {
			const int futilityMargin = Tune::tune_fp_margin_offset() + depth * Tune::tune_fp_margin_coeff();
	...
	while (movePicker.HasNext()) {
		...
		if (depth >= 3 && (legalMoveCount >= (3 + pvNode * 2 + rootNode * 2)) && isQuiet) {
			...
			int reduction = static_cast<int>((Tune::tune_lmr_multiplier() / 100.0) * std::log(std::min(depth, 31)) * std::log(std::min(failLowCount, 31)) + (Tune::tune_lmr_base() / 100.0));
			if (!ttPV) reduction += 1;
			if (t.CutoffCount[level] < 4) reduction -= 1;
			if (std::abs(order) < 80000) reduction -= std::clamp(order / Tune::tune_lmr_history_div(), -2, 2);
			...
			if (score > alpha && reducedDepth < depth - 1) {
				deepen = score > (bestScore + Tune::tune_lmr_deeper_offset() + (depth - 1) * Tune::tune_lmr_deeper_coeff());
