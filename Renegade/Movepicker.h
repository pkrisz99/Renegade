...
public:
	...
	void Initialize(const MoveGen moveGen, const Position& pos, const Histories& hist, const Move& ttMove, const int level) {
		...
		this->materialKeyMove = hist.GetMaterialKeyMove(pos);
		this->level = level;
...
private:
	...
	int GetMoveScore(const Position& pos, const Histories& hist, const Move& m) const {
		...
		else if (m == positionalMove || m == materialKeyMove) historyScore += 16384;

		return historyScore;
	}


	Move ttMove{}, killerMove{}, counterMove{}, positionalMove{}, materialKeyMove{};
	MoveList moves{};
	int level = 0;
