	...
	int GetMoveScore(const Position& pos, const Histories& hist, const Move& m) const {
		...
		if (capturedPieceType != PieceType::None) {
			...
			const bool losingCapture = [&] {
				...
				return !pos.StaticExchangeEval(m, -captureScore / 34);
			}();

			if (!losingCapture) return 600000 + values[capturedPieceType] * 17 + hist.GetCaptureHistoryScore(pos, m);
			else return -200000 + values[capturedPieceType] * 17 + hist.GetCaptureHistoryScore(pos, m);
		}
