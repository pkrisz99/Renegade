...
public:
	...
	void Initialize(const MoveGen moveGen, const Position& pos, const Histories& hist, const Move& ttMove, const int level) {
		...
		this->threatMove = hist.GetThreatRefutationMove(pos);
		this->level = level;
...
private:
	...
	int GetMoveScore(const Position& pos, const Histories& hist, const Move& m) const {
		...
		else if (m == positionalMove || m == threatMove) historyScore += 16384;

		return historyScore;
	}


	Move ttMove{}, killerMove{}, counterMove{}, positionalMove{}, threatMove{};
	MoveList moves{};
	int level = 0;
