	...
	int GetMoveScore(const Position& pos, const Histories& hist, const Move& m) const {
		...
		if (capturedPieceType != PieceType::None) {
			...
			const bool losingCapture = [&] {
				...
				return !pos.StaticExchangeEval(m, -captureScore / Tune::tune_capthist_div());
			}();

			if (!losingCapture) return 600000 + values[capturedPieceType] * Tune::tune_capthist_mul() + hist.GetCaptureHistoryScore(pos, m);
			else return -200000 + values[capturedPieceType] * Tune::tune_capthist_mul() + hist.GetCaptureHistoryScore(pos, m);
		}
