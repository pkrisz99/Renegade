...
int Heuristics::CalculateOrderScore(const Board& board, const Move& m, const int level, const Move& ttMove,
	const std::array<MoveAndPiece, MaxDepth>& moveStack, const bool losingCapture, const bool useMoveStack, const uint64_t opponentAttacks, const uint64_t friendlyAttacks) const {
	...
	const int b1 = CheckBit(opponentAttacks, m.from) + 2 * CheckBit(friendlyAttacks, m.from);
	const int b2 = CheckBit(opponentAttacks, m.to) + 2 * CheckBit(friendlyAttacks, m.to);

	int historyScore = HistoryTables[b1][b2][movedPiece][m.to];
...
void Heuristics::UpdateHistory(const Move& m, const int16_t delta, const uint8_t piece, const int depth, const std::array<MoveAndPiece, MaxDepth>& moveStack,
	const int level, const bool fromSquareAttacked, const bool toSquareAttacked, const bool fromSquareDefended, const bool toSquareDefended) {

	// Main quiet history
	const bool side = ColorOfPiece(piece) == PieceColor::White; 
	UpdateHistoryValue(HistoryTables[fromSquareAttacked + 2*fromSquareDefended][toSquareAttacked + 2*toSquareDefended][piece][m.to], delta);
