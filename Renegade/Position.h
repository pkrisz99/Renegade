...
public:
	...
	inline uint64_t GetPreviousPawnKey() const {
		const Board& prevState = States[States.size() - 2];
		return MurmurHash3(prevState.WhitePawnBits) ^ MurmurHash3(prevState.BlackPawnBits ^ Zobrist[780]);
	}
